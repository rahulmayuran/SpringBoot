...
*  Kafka is a distributed streaming platform because it has 1,2…n nodes or Brokers that could be run across multiple regions & data centers, can build location tracker!. 
		a. It is fault tolerant, highly scalable as it runs as cluster
		b. Kafka Broker is a VM that runs kafka process that can run local, in cloud
		c. Kafka Clusters : Can have multiple brokers, each broker has multiple combos of topics and partitions. These clusters communicate to producer and consumer
		d. producer sends a stream of data to the broker as topics in the form of events
		e. Kafka Connect Source for data integration which pulls data from your existing data source 
		f. Kafka Connect Sink takes data from topics and puts to data source
		g. Kafka Stream API is used for data processing, filtering, grouping, aggregating etc
		h. Topic segregates the stream of messages produced by different producers, It is a collection of events which is replicated and partitioned, Is analogous to log. The data wont disappear, they can stay for couple of minutes/ hours. There is no size limit for a topic, Can have one node for a big topic
		i. Partitions are mapped to consumers that has indexes starting from 0.
		j. Consumer can consume data of this partition from offset.
		k. Offset is a location where consumer reads message from partition.
		l. If there are 3 partition and 4 consumers , then the 4th consumer wont be able to receive any message. But reverse is not same, So the No of Partitions is always > No of consumers.
		
	2. Kafka needs bootstrap-servers, key serializer and value serializer under producer
Zookeeper is used to track the status of Kafka messages and has the cluster information. 
		a. Kafka by default runs in port 9092.
		b. Zookeeper by default runs in port 2181.
		c. Make sure System variables have Jdk path's bin folder correctly set. 
		d. Also set ZOO_HOME and KAFKA_HOME in the same way and change the file to zoo.cfg instead of zoo_order default file.
		e. Run zookeeper first in 2181 ,then run kafka as it creates a session id, cluster id
		...
		f. Then, It creates a log directory under tmp, as per the property file.
		...
		g. Then kafka server gets started
		...
		h. Define a @Configuration class with a @Bean method of NewTopic to create the topic fse_stock and run the microservice , add spring.kafka.bootstrap-servers=localhost:9092 to application-properties file. 
		...
		i. Now , in the console of microservice, you can find below message
		...
		j. Write the configurations for producer with the help of producerConfig map having key and value serializers, ProducerFactory which takes producerconfig as arg and kafkaTemplate which takes producerFactory instance, all marked with @Bean.
		k. The moment you produce these messages, you can check from cmd for the reception of these messages with below command for consumer.
		l. To consume in code ,Similarly write consumer logic with the above pattern followed for producer except for the fact that, consumer uses KafkaListenerContainerFactory with all big names. 
		m. Now, to get the messages that producer and consumer are talking, we need a listener. Do similar approach, but this time mark it as @Component instead of @Configuration and just listen to the data that holds the topics and group id with @Kafkalistener. 
		...
		Reference : https://github.com/java2786/cts_batch_5/tree/master/kafka 
		Kafka setup - https://www.devglan.com/corejava/apache-kafka-java-example
		Kafka and Docker - https://www.kaaiot.com/blog/kafka-docker
		
	Kafka Commands

	3. Start zookeeper - $ ./bin/windows/zookeeper-server-start.bat ./config/zookeeper.properties Or in case of higher version, simply type zkServer in cmd after setting zookeeper path. 

	4. Start kafka - $ .\bin\windows\kafka-server-start.bat .\config\server.properties 

	5. Create topic 
		a. To create 1 topic with 1 partition -> .\bin\windows\kafka-topics.bat --create --zookeeper localhost:2181 --replication-factor 1 --partitions 1 --topic kafka_topic_name
		b. To create 1 topic with 3 partitions -> .\bin\windows\kafka-topics.bat --create --zookeeper localhost:2181 --replication-factor 1 --partitions 3 --topic threePartitions

	6. List topics - $ .\bin\windows\kafka-topics.bat --list --zookeeper localhost:2181

	7. Delete topic -
		a.  Go to kafka/config/server.properties and add delete.topic.enable=true
		b. Execute this command in cmd -> .\bin\windows\kafka-topics.bat --zookeeper localhost:2181 -topic stock_topic --delete. This is marked for deletion because I used _ in topic.

	8. Produce into topic - $ .\bin\windows\kafka-console-producer.bat --broker-list localhost:9092 --topic kafka_topic

9. Consume from topic -$ .\bin\windows\kafka-console-consumer.bat --bootstrap-server localhost:9092 --topic kafka_topic --from-beginning!
